...
from .Items import RealmItem, item_data_table, item_table, code_to_item_table
from .Locations import RealmLocation, location_data_table, location_table, code_to_location_table, locked_locations
from .Options import RealmOptions
from .Regions import region_data_table, get_exit
from .Rules import *


class RealmWebWorld(WebWorld):
    # ~ theme = "partyTime"
    
    setup_en = Tutorial(
        tutorial_name="Start Guide",
        description="A guide to playing Realm of the Mad God in Archipelago.",
        language="English",
...
class RealmWorld(World):
    """The co-op fantasy mmo shooter."""

    game = "Realm of the Mad God"
    data_version = 1
    web = RealmWebWorld()
    options_dataclass = RealmOptions
    ...
    def create_item(self, name: str) -> RealmItem:
        return RealmItem(name, item_data_table[name].type, item_data_table[name].code, self.player)

    def create_items(self) -> None:
        mw = self.multiworld

        item_pool: List[RealmItem] = []
